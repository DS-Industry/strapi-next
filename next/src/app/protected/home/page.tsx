...
import { CommentAttributes, StrapiData, TaskAttributes } from "@/types/types";
...
export default async function Home () {
    ...
    const [ { data  : openTask } , { data : workTask }, { data: deadlineOverTask }, {data: commentData}] = await Promise.all([
        ...
        headers
        }), axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/tasks?filters[$and][0][status][id][$lt]=8&filters[asiignees][id][$in]=${session?.user.id}`, {
        headers
        }), axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/comments?populate[createdUserBy][populate][0]=avatar&populate=task&filters[$and][1][createdUserBy][id][$eq]=${session?.user.id}`, {
            headers
        })
    ]);
        ...
        <main>
            ...
            <div>
                <div className=" flex flex-wrap gap-5 w-full mt-5">
                    ...
                    <Card count={deadlineOverTask.data.filter((task : StrapiData<TaskAttributes>) => {
                        const today  = new Date().getTime();
                        const deadline = new Date(task.attributes.deadline).getTime()
                        return task.attributes.type === 'Задача' && deadline < today
                        }).length} label="Просроченные задачи" icon={<MdLocalFireDepartment />} attention={true}/>
                    ...
                    <Card count={deadlineOverTask.data.filter((task : StrapiData<TaskAttributes>) => {
                        const today  = new Date().getTime();
                        const deadline = new Date(task.attributes.deadline).getTime();
                        return task.attributes.type === 'Обращение' && deadline < today
                        }).length} label="Просроченные обращения" icon={<MdLocalFireDepartment />} attention={true} />
                </div>
            </div>
            <div className="w-full flex gap-10">
                <div className=" w-1/2 mt-10">
                    <div className=" rounded-xl border bg-white border-white p-3">
                        ...
                        <CommentList comments={commentData.data.filter((comment: StrapiData<CommentAttributes>) => {
                            const today = new Date();
                            const treeDaysAgo = new Date().setDate(today.getDate() - 3);
                            const createdAt = new Date(comment.attributes.createdAt).getTime();
                            return createdAt > treeDaysAgo
                        })} homePage={true}/>
                    </div>
                </div>
{/*                 <div className=" w-1/2 mt-10">
