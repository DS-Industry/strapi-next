...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    <aside
      ref={sidebar}
      className={`absolute left-0 z-9 top-0 flex h-screen w-72.5 flex-col overflow-y-hidden bg-black duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
        sidebarOpen ? "translate-x-0" : "-translate-x-full"
      }`}
    >
      {/* <!-- SIDEBAR HEADER --> */}
      <div className="flex items-center justify-start gap-2 ml-10  lg:py-6.5">
        <Link href="/protected/home" className=" w-full items-center text-2xl text-bodydark1">
          HELP DESK
        </Link>
      ...
      <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
        {/* <!-- Sidebar Menu --> */}
        <nav className=" py-4 px-1 lg:px-6">
          {/* <!-- Menu Group --> */}
          <div>
            ...
            <ul className="mb-6 flex flex-col gap-1.5 exclude-list-styling">
              ...
              <li>
                <Link
                  ...
                  className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${(pathname === "/protected/tasks") &&
                    "bg-graydark dark:bg-meta-4"
                  }`}
                >
                <div className=" text-xl">
              ...
              <li>
                <Link
                  ...
                  className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    (pathname === "/protected/todos") && "bg-graydark dark:bg-meta-4"
                  }`}
                >
                  <div className=" text-xl">
              ...
              <SidebarLinkGroup
                activeCondition={
                 pathname === "/protected/tasks/create" || pathname === "/protected/todos/create" || pathname.includes("todos/create") || pathname.includes("tasks/create")
                }
             >
                {(handleClick, open) => {
                 return (
                    <React.Fragment>
                     <Link
                        href="/protected/tasks/create"
                        className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                         (pathname === "/protected/tasks/create" ||
                            pathname.includes("tasks/create")|| 
                            pathname.includes("todos/create")) &&
                         "bg-graydark dark:bg-meta-4"
                        }`}
                        onClick={(e) => {
                         e.preventDefault();
                     ...
                     >
                        <ul className="mt-4 mb-5.5 flex flex-col gap-2.5 pl-6 exclude-list-styling">
                         <li>
                            <Link
                             href="/protected/tasks/create"
                             className={`first-letter:group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-bodydark2 duration-300 ease-in-out hover:text-white ${
                                pathname === "/protected/tasks/create?type=Обращение" &&
                                "text-white"
                             }`}
                            >
                             Обращение
                        ...
                        {session?.user.role.name !== 'TechnicalService' &&
                            <li>
                              <Link
                              href="/protected/instruction"
                              className={`group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-bodydark2 duration-300 ease-in-out hover:text-white ${
                                  pathname === "/protected/instruction" &&
                                  "text-white"
                              }`}
                              >
                              Обращение по инстукции
                              </Link>
                          </li>
                         }
                         <li>
                            <Link
                             href="/protected/todos/create?type=Задача"
                             className={`group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-bodydark2 duration-300 ease-in-out hover:text-white ${
                                pathname === "/protected/todos/create?type=Задача" &&
                                "text-white"
                             }`}
                            >
                             Задачу
...
{/*              <!-- Menu Item User creation --> *
              ...
              <li>
                <Link
                  ...
                  className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    pathname.includes("profile") && !pathname.includes("create") && "bg-graydark dark:bg-meta-4"
                  }`}
                >
                  <svg
                    className="fill-current"
