import DropdownButton from "@/components/client/buttons/drop-down-button";
...
export default async function TicketListPage () {
    ...
    return (
/*         <main>
            <p classNameName=" text-4xl text-black">
                Department {user.department && user.department.name}
            </p>
            <table classNameName=" bg-bodydark1 text-body  w-full mt-4">
                <thead>
                    <tr>
                        <th>New window</th>
                        <th>Key</th>
                ...
                <tbody>
                    {
                        data.data.map((ticket : any, index: number) => {
                            return (
                                <tr key={index}>
                                    <td>{ticket.id}</td>
                                    <td>{ticket.attributes.slug}</td>
                                    <td>{ticket.attributes.name}</td>
                                    <td>{ticket.attributes.createdUserBy.data.attributes.username}</td>
                                    <td>{ticket.attributes.name}</td>
                                    <td>{ticket.attributes.carWash.data.attributes.slug}</td>
                                    <td>{ticket.attributes.priority.data.attributes.name}</td>
                                    <td>{ticket.attributes.status.data.attributes.name}</td>
                                    <td>{ticket.attributes.createdAt}</td>
                                </tr>
                                )})
                    }
